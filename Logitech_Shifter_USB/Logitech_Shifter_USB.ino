...
void loop() {
  ...
  if (_isreverse == 1 && _gear_ == 6) _gear_ = 7; // Reverse gear is allowed only on 6th gear position
...
void desactivar() {
  ...
  controllerData = getBlankDataForController();
  bool state = false;
  controllerData.triangleOn = state;
  controllerData.circleOn = state;
  controllerData.squareOn = state;
  controllerData.crossOn = state;
  controllerData.dpadUpOn = state;
  controllerData.dpadDownOn = state;
  controllerData.dpadLeftOn = state;
  setControllerData(controllerData);
}
void setBtn(int btn, bool state) {
  controllerData = getBlankDataForController();
  switch (btn) {
    case 1:
      controllerData.triangleOn = state;
